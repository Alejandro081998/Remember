...
public class AdaptadorNotificaciones extends ArrayAdapter<Notificaciones> implements Filterable {

    protected List<Notificaciones> objetos;
    CustomFilter filter;
    List<Notificaciones> filterlist;

    public AdaptadorNotificaciones(Context context, List<Notificaciones> objetos) {
        super(context, R.layout.fragment_notificaciones, objetos);
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(getItem(position).getEstado().getId_estado() == 2){

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
                marco.setBackgroundColor(Color.parseColor("#E0F7FA"));
            }
